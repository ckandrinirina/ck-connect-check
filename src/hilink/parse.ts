...
/**
 * The router's code for "that token has been used" — the cue to refresh the
 * token, not the session. A `POST` spends the token it carries, so this is what
 * a second write with the same one earns.
 */
...
export function readReply(xml: string, endpoint: string): Map<string, string> {
  ...
  if (root === "error") {
    ...
    if (code === undefined || !/^-?\d+$/.test(code)) {
      throw new HilinkParseError(
        endpoint,
        "error reply without a numeric <code>",
      );
    }
...
function parseDataLimitBytes(value: string, endpoint: string): number {
  ...
  if (!match || scale === undefined) {
    throw new HilinkParseError(
      endpoint,
      `<DataLimit> is not a data size: "${value}"`,
    );
  }
...
/**
 * The current verification token, for when a reply carried no token header. The
 * device pads the element, and only its last 32 characters are the value it
 * accepts back.
 */
export function parseToken(xml: string): string {
  const endpoint = "/api/webserver/token";
  const fields = readReply(xml, endpoint);
  return requireText(fields, "token", endpoint).slice(-TOKEN_LENGTH);
}
...
export function parseStatus(xml: string): RouterStatus {
  ...
  return {
    connected:
      requireNumber(fields, "ConnectionStatus", endpoint) === CONNECTED_STATUS,
...
export function parseStartDate(xml: string): BillingCycle {
  ...
  return {
    ...
    routerDataLimitBytes: parseDataLimitBytes(
      requireText(fields, "DataLimit", endpoint),
      endpoint,
    ),
