...
/**
 * One device, already spelled the way it appears on the Devices pane.
 *
 * There is no medium column and no active dot, which the list was sketched
 * with: T-62's probe found no band, frequency, medium or `Active` element in
 * `host-list`, and the endpoint reports only the hosts currently associated —
 * so an active dot would be lit on every row it drew. {@link DeviceRow.network}
 * is `AssociatedSsid` shown as itself, which is the one network field the
 * router really sends.
 */
export interface DeviceRow {
  ...
  mac: string;
  network: string;
  /** Empty for a device the router is not reporting; the pane says so itself. */
  connectedFor: string;
   ...
   * the local interfaces. The pane withholds the block control on a row that
 ...
 * replaced: a {@link DeviceBlockRefusal} is settled before any request goes out,
 * and a {@link RouterFailure} is the router's own word on one that did —
 * including a numeric code nobody has named, carried whole as a `RouterRefusal`
 * with the endpoint that produced it.
 *
 * Every member is either a word or an object with a `kind`, so one `typeof` and
 * one `kind` tell all of them apart. There is deliberately no third vocabulary
 * and no "something went wrong" catch-all: a refusal the pane cannot name is
 * exactly the one whose number matters most.
 */
export type DeviceRefusal = DeviceBlockRefusal | RouterFailure;

/**
 * What the Devices pane shows, in the three shapes it can be in.
 *
 ...
 *   the router says nothing is connected — and the pane states it as such.
 * - `offline` is the router not having answered at all, which is a normal state
 *   rendered like the panel's figures, never an error dialog: the app runs
 *   unattended.
 ...
 *   router for something the user settles in the settings view in ten seconds.
 *
 * {@link DevicesModel.refusal} rides the list rather than replacing it, and that
 * is the point: a write that failed says nothing about which devices are
 * connected, so emptying the list over one would throw away the very thing the
 * pane exists to show.
 *
 * The offline *reason* deliberately does not travel, exactly as it does not
 * reach the figures — an unreachable router is one state however it is
...
/** One device, spelled for a row. */
function deviceRowFor(listed: ListedDevice): DeviceRow {
  const { device } = listed;
...
/**
 * The pane's contents for one reading of the host list and one of the MAC
 * filter.
 *
 * The ordering is the domain's — stable across polls, so a row cannot move under
 * a click — and so are the naming, the duration and the blocked verdict. This
 ...
 * password and a second call, so the pane must be able to list devices before
 ...
 * list rather than shown instead of it. A refusal reaching an unreachable router
 * is dropped: "the router is not answering" is the larger and truer statement,
 * and stacking a second complaint on top of it says nothing more.
 */
export function buildDevicesModel(
  ...
  const devices = listDevices(result.devices, filter, localMacs).map(
    deviceRowFor,
  );
