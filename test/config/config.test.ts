...
describe('corrupt config', () => {
  ...
  it('falls back to defaults when the JSON holds an invalid value', () => {
    writeFileSync(path(), JSON.stringify({ ...defaultConfig(), pollIntervalSeconds: 1 }));
...
describe('plan limit in GB', () => {
  ...
  it('persists the converted byte value, not the GB value', () => {
    saveConfig(path(), { ...defaultConfig(), planLimitBytes: gigabytesToBytes(20) });

    expect(JSON.parse(readFileSync(path(), 'utf8')).planLimitBytes).toBe(20_000_000_000);
  });
});
...
describe('plan limit validation', () => {
  ...
  it('rejects a non-numeric plan limit', () => {
    ...
    expect(() => gigabytesToBytes(Number.POSITIVE_INFINITY)).toThrow(ConfigValidationError);
  });

  it('rejects a non-numeric plan limit coming from a config object', () => {
    expect(() => parseConfig({ ...defaultConfig(), planLimitBytes: 'twenty' })).toThrow(
      ConfigValidationError,
    );
  });
...
describe('poll interval validation', () => {
  it('rejects a poll interval below 5 seconds so the router is not hammered', () => {
    expect(() => parseConfig({ ...defaultConfig(), pollIntervalSeconds: 4 })).toThrow(
      ConfigValidationError,
    );
  });
  ...
  it('names pollIntervalSeconds on the rejection', () => {
    try {
      ...
      expect((error as ConfigValidationError).field).toBe('pollIntervalSeconds');
    }
  });

  it('accepts exactly the 5 second floor', () => {
    expect(parseConfig({ ...defaultConfig(), pollIntervalSeconds: 5 }).pollIntervalSeconds).toBe(5);
  });

  it('rejects a non-numeric poll interval', () => {
    expect(() => parseConfig({ ...defaultConfig(), pollIntervalSeconds: '30' })).toThrow(
      ConfigValidationError,
    );
  });

  it('rejects an empty host', () => {
    expect(() => parseConfig({ ...defaultConfig(), host: '   ' })).toThrow(ConfigValidationError);
  });

  it('rejects a warn threshold outside 1–100', () => {
    expect(() => parseConfig({ ...defaultConfig(), warnThresholdPercent: 101 })).toThrow(
      ConfigValidationError,
    );
    expect(() => parseConfig({ ...defaultConfig(), warnThresholdPercent: 0 })).toThrow(
      ConfigValidationError,
    );
  });
});
